...
void MachineInstr::print(raw_ostream &OS, const TargetMachine *TM,
  ...
  if (Flags & PrintableFlags) {
    ...
    std::error_code ec;
    MachineMemOperand *tmp_mmo;
    MachineMemOperand *tmp_mmo_bool;
    ...
    bool mem_flag = false;
    bool mem_flag_bool = false;
    std::string op_name;
    std::string op_name_complete;
    // check the opcode name.
    if (TM && TM->getSubtargetImpl()->getInstrInfo())
    {
        const char *tmp = TM->getSubtargetImpl()->getInstrInfo()->getName(getOpcode());
        std::string str(tmp);
        op_name_complete.assign(str);
        op_name = op_name_complete.substr(0,3);
    }
    ...
    for (unsigned i = 0; i != VirtRegs.size(); ++i) {
      ...
      if((!op_name.compare("XOR")) || (!op_name.compare("MOV")))
      {
          ...
          if(!left_side)
          {
              ...
              (*outFile) << op_name;
              (*outFile) << " ";
              // print posseible mem operands
              if(mem_flag)
              {
                  // print memory frame information
                  (*outFile) << *tmp_mmo;
                  (*outFile) << ' ';
              }
              left_side = true;
          ...
          // for factInt version or bool version
          if(mem_flag_bool)
          {
              (*outFile3) << *tmp_mmo_bool;
              (*outFile3) << ' ';
              // for bool version
              ...
              (*outFile2) << ' ';
              // for bool version
              (*outFile2) << PrintReg(VirtRegs[i]);
              (*outFile2) << "\n";
          }
            // print register information
              (*outFile) << PrintReg(VirtRegs[i]);
              //(*outFile) << "\n";
              (*outFile) << " ";
      }
      // bobo ends
