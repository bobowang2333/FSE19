...
// bobo: init the reserved register list for the sensitive variables
void RegisterClassInfo::leakPhysRegInit(std::set<unsigned> * leakPhysReg) const
{
    // insert two physical registers -> R10, R11(32 bit)
    leakPhysReg->insert(228);
    leakPhysReg->insert(229);
}
...
void RegisterClassInfo::compute(const TargetRegisterClass *RC) const {
  ...
  for (unsigned i = 0; i != RawOrder.size(); ++i) {
    ...
    // bobo: Remove reserved registers for the sensitive variables
    /*
    std::set<unsigned> leakPhysReg;
    std::set<unsigned>::iterator it;
    leakPhysRegInit(&leakPhysReg);
    if(leakPhysReg.find(PhysReg) != leakPhysReg.end())
        continue;
     */
