...
#define DEBUG_TYPE "checkHint"
  ...
  if (MRI.isReserved(Phys))
    ...
    // bobo edit the hint allocation for the sensitive register
    // temporarily comment
    /*
    unsigned stop_coalescing = 5; // reg_index which is not allowed to coalesce
    unsigned reg_index = TargetRegisterInfo::virtReg2Index(VirtReg);
    DEBUG(dbgs() << "Modify the hint of Reg: %vreg" << reg_index << "\n");
    if(reg_index == (--stop_coalescing))
    {
        // iterate the allocation order and eliminate the physical register which has been assigned to the previous %vreg
        for(auto it = Order.begin(); it != Order.end();++it)
        {
            if(it == std::find(Order.begin(), Order.end(), Phys))
                continue;
            else
                Hints.push_back(*it);
        }
