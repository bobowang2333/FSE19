...
NodeDone:
  ...
  for (SelectionDAG::allnodes_iterator I = DAG.allnodes_begin(),
    ...
    if(!(fin.peek() == std::ifstream::traits_type::eof())) // forbackend.txt file is not empty
    {
        fin.close();
        Modify_xor();
    }
...
//bobo (1) change the store address of xor instruction
// (2) change the following instruction which will use this xor instruction results
void DAGTypeLegalizer::Modify_xor(void)
{
    //check if the two properties of the instruction (1) load (2) has %key as operand
    //change the store address of the xor instruction result
    SelectionDAG::allnodes_iterator tmp_stop;
    SDValue key_mem;
    for (SelectionDAG::allnodes_iterator I = DAG.allnodes_begin(),
         E = DAG.allnodes_end(); I != E; ++I)
    {
        ...
        {
            // print the mem operand
            std::string InterVar_name = showName(sdnToCheck);
            std::cout << InterVar_name << std::endl;
            //check if it contains key variable
            for(unsigned i_store = 0, e_store = sdnToCheck->getNumOperands(); i_store != e_store; ++i_store)
            {
                unsigned long long key_address = (uintptr_t) sdnToCheck->getOperand(i_store).getNode();
                // get the representation of "n2" find the corresponding instr
                // modify the mem address of the instr
                std::cout << "Checking: " << showAddress(key_address) << std::endl;
                //bobo newly added
                if(i_store == 2)
                {
                    std::string s;
                        while(fin >> s)
                        {
                            if(!s.compare("HD_SENSITIVE"))
                            {
                                ...
                                {
                                    SDValue from = sdnToCheck->getOperand(i_store); // third operand for "store" instr
                                    SelectionDAG::allnodes_iterator tmp_i = I;
                                    tmp_stop = I;
                                    SDNode *lastLoad = I;
                                    std::string lastLoad_name = lastLoad->getOperationName(&DAG); // operation name
                                    std::string lastLoad_var;
                                    fin >> s;
                                    while(true)
                                    {
                                        --tmp_i;
                                        lastLoad = tmp_i;
                                        lastLoad_name = lastLoad->getOperationName(&DAG);
                                        if(!lastLoad_name.compare("load"))
                                        {
                                            lastLoad_var = showName(lastLoad);
                                            //(x1, x0.addr) skip "load x0.addr" we can get the load key instr
                                            if(lastLoad_var.compare(s))
                                                break;
                                        }
                                    }
                                    SDNode *sdnToEdit = tmp_i;
                                    std::string checkOpcode = sdnToEdit->getOperationName(&DAG);
                                    std::cout << checkOpcode << std::endl;
                                    SDValue to = sdnToEdit->getOperand(i_store-1); // second operand for "load" instr
                                    key_mem = to;
                                    //show the address of the previous load instr
                                    unsigned long long load_address = (uintptr_t) sdnToEdit->getOperand(i_store-1).getNode();
                                    std::cout << "Loading address to replace store is: " << showAddress(load_address) << std::endl;
                                    DAG.ReplaceAllUsesOfValueWith(from, to);
                                    /** stop the outest iteration **/
                                    // stop_mark = 1;
                                    /** stop the second outest iteration **/
                                    break;
                                }
                            }
                        }
    ...
    // after changing the store address, change the following instr which will use the result of previous xor instr
    // add a load instr which loads value n2 from the FrameIndex
    
    // DAG.ReplaceAllUsesOfValueWith can replace the following load instr from modified store address instead of the original address
    /*
    for(SelectionDAG::allnodes_iterator I = tmp_stop, E = DAG.allnodes_end();I != E; ++I)
    {
        //bobo newly add
        std::string s;
        std::ifstream fin("forBackend.txt");
            ...
            while(fin >> s)
            {
                ...
                if(!s.compare("HD_SENSITIVE"))
                {
                    ...
                    if(!InterVar_name.compare(s))
                    {
                        SelectionDAG::allnodes_iterator tmp_i = I;
                    //++tmp_i;
                    //SDNode *leak_and = tmp_i;
                    //SDValue NewLoadNode = DAG.getNode(sdnToCheck->getOpcode(), SDLoc(sdnToCheck), sdnToCheck->getOperand(0).getValueType(), sdnToCheck->getOperand(0), key_mem, sdnToCheck->getOperand(2));
                    //DAG.ReplaceAllUsesOfValueWith(leak_and->getOperand(), NewLoadNode);
                    //leak_and = DAG.UpdateNodeOperands(leak_and, NewLoadNode, leak_and->getOperand(1));
                    // replace the FrameIndex of %n2 with the FrameIndex of %key
                        SDValue load_n2 = sdnToCheck->getOperand(1);
                        unsigned long long load_address = (uintptr_t) sdnToCheck->getOperand(1).getNode();
                        std::cout << "Loading address to replace store is: " << showAddress(load_address) << std::endl;
                        DAG.ReplaceAllUsesOfValueWith(load_n2, key_mem);
                    }
                }
            }
        }
...
// swap the load order of %n2 and %i3
// TO BE FINISHED
/*
void DAGTypeLegalizer::swap_load_order(void)
{
    SDValue mem_n2, mem_i3, tmp;
    SDNode *node_n2, *node_i3;
    for(SelectionDAG::allnodes_iterator I = tmp_stop, E = DAG.allnodes_end();I != E; ++I)
    {
        SDNode *nodeToSwap = I;
        if(!nodeToSwap)
            return;
        std::string checkOpcode = nodeToSwap->getOperationName(&DAG);
        if(checkOpcode.compare("load") == 0)
        {
            std::string mem_op = showName(nodeToSwap);
            std::cout << mem_op << std::enl;
            if(!mem_op.compare("n2"))
            {
                mem_n2 = nodeToSwap->getOperand(1);
                node_n2 = nodeToSwap;
            }
            else if(!mem_op.compare("i3.addr"))
            {
                mem_i3 = nodeToSwap->getOperand(1);
                node_i3 = nodeToSwap;
            }
            else
            {}
    ...
    DAG.ReplaceAllUsesOfValueWith(mem_i3, tmp);
    DAG.ReplaceAllUsesOfValueWith(mem_n2);
...
/*
 ...
 {
 unsigned long long key_address = (uintptr_t) sdnToCheck->getOperand(i_store).getNode();
 // in the "store" instruction, modify the second(2) address to the key's storing memory address
 //n2 = "1068470c0" FrameIndex<5>
 //key = "1068450e0" FrameIndex<2>
 // get the representation of "n2" find the corresponding instr
 
 // modify the mem address of the instr
 std::cout << "Checking: " << showAddress(key_address) << std::endl;
 // temporarily comment
 if(!InterVar_name.compare("n2") && (i_store == 2))
 {
 SDValue from = sdnToCheck->getOperand(i_store); // third operand for "store" instr
 SelectionDAG::allnodes_iterator tmp_i = I;
 --tmp_i;
 ...
 else if(checkOpcode.compare("load") == 0)
 {
 std::string InterVar_name = showName(sdnToCheck);
 std::cout << InterVar_name << std::endl;
 if(!InterVar_name.compare("i3.addr"))
 {
 SelectionDAG::allnodes_iterator tmp_i = I;
 --tmp_i;
 --tmp_i;
 --tmp_i;
 SDNode *leak_xor = tmp_i;
 //SDNode *NewLoadNode = sdnToCheck;
 //NewLoadNode = DAG.UpdateNodeOperands(sdnToCheck, sdnToCheck->getOperand(1), key_mem);
 SDValue NewLoadNode = DAG.getNode(sdnToCheck->getOpcode(), SDLoc(sdnToCheck), sdnToCheck->getOperand(0).getValueType(), sdnToCheck->getOperand(0), key_mem, sdnToCheck->getOperand(2));
 DAG.ReplaceAllUsesWith(leak_xor, NewLoadNode.getNode());
 //EnforceNodeIdInvariant(NewLoadNode);
 //for (unsigned i = 0, e = leak_xor->getNumValues(); i != e; ++i)
 //    DAG.TransferDbgValues(SDValue(leak_xor, i), SDValue(NewLoadNode, i));
 }
 }
*/

//bobo
//show the string form "%i1.addr" of the instruction
...
std::string DAGTypeLegalizer::showName(SDNode *sdnToCheck)
{
    ...
    const Value *V_p = MMO->getValue();
    StringRef InterVar = V_p->getName();
    std::string InterVar_name(InterVar);
    return InterVar_name;
}

//bobo
// transform the address form (from long to the string(hex)) 0x4497491
std::string DAGTypeLegalizer::showAddress(unsigned long long key_address)
{
    ...
    while (key_address) {
        uintptr_t x = key_address % 16;
        *--CurPtr = (x < 10 ? '0' + x : 'a' + x - 10);
        key_address /= 16;
    }
    std::string str_address(CurPtr);
    return str_address;
}
