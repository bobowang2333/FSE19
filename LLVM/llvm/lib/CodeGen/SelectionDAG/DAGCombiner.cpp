...
//------------------------------ DAGCombiner ---------------------------------//
  ...
  private:
    ...
    //bobo
      std::string showName(SDNode *sdnToCheck);
...
void DAGCombiner::Run(CombineLevel AtLevel) {
  ...
  while (!WorklistMap.empty()) {
    ...
    //bobo
    // before delete print the opcode
    /*
    std::string checkOpcode = N->getOperationName(&DAG);
    std::cout << checkOpcode << std::endl;
    if(!checkOpcode.compare("load"))
    {
        std::string addr = showName(N);
...
std::string DAGCombiner::showName(SDNode *sdnToCheck)
{
    ...
    //bobo: newly comment
    //if(!((V_p->getType())->isPointerTy()))
    //{
        StringRef InterVar = V_p->getName();
        std::string InterVar_name(InterVar);
        return InterVar_name;
    //}else{
      //  std::string InterVar_name= "this load/store operand doesn't have name";
        //return InterVar_name;
    //}
}
...
SDValue DAGCombiner::visitLOAD(SDNode *N) {
  ...
  if (ISD::isNormalLoad(N) && !LD->isVolatile()) {
    if (ISD::isNON_TRUNCStore(Chain.getNode())) {
     ...
     //bobo
      {
          std::string checkAddr = showName(N);
          std::cout << checkAddr << std::endl;
          std::string s;
          std::ifstream fin("forBackend.txt");
          bool combine_flag = true;
          if((fin.peek() == std::ifstream::traits_type::eof())) // "forBackend" file is empty
