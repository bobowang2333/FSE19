...
SelectCodeCommon(SDNode *NodeToMatch, const unsigned char *MatcherTable,
  ...
  while (1) {
    ...
    case OPC_MorphNodeTo: {
        //bobo
        /*
        unsigned xor32rm_index = 94656;
        unsigned xor32mr_index = 4976;
      if(MatcherIndex == xor32rm_index)
          MatcherIndex = xor32mr_index;
         */
      uint16_t TargetOpc = MatcherTable[MatcherIndex++];
      TargetOpc |= (unsigned short)MatcherTable[MatcherIndex++] << 8;
      std::cout << TargetOpc << std::endl;
      ...
      if (Opcode != OPC_MorphNodeTo) {
        ...
        for (unsigned i = 0, e = VTs.size(); i != e; ++i) {
          ...
          std::cout << NodeToMatch->getOpcode() << std::endl;
        Res = MorphNode(NodeToMatch, TargetOpc, VTList, Ops, EmitNodeInfo);
          std::cout << Res->getMachineOpcode() << std::endl;
      } else {
        ...
        //bobo
        // print the generated machine opcode
        const TargetInstrInfo *TII = CurDAG->getSubtarget().getInstrInfo();
        if(Res->getMachineOpcode() < TII->getNumOpcodes())
        {
            std::string P_opcode = TII->getName(Res->getMachineOpcode());
            std::cout << P_opcode << std::endl;
        }
    ...
    while (1) {
      ...
      //temporarily comment
      /*
      unsigned scope_ind_rm = 94656;
      unsigned scope_ind_mr = 4976;
        if(MatcherIndex == scope_ind_rm)
            MatcherIndex = scope_ind_mr;
      */
