...
void VirtRegMap::print(raw_ostream &OS, const Module*) const {
  ...
  //bobo
    ...
    std::string path2 = "memBool.log";
    std::error_code ec;
    // clear the previous content, and only reserve the register map info
    raw_fd_ostream *clearFile = new raw_fd_ostream(path.c_str(), ec, sys::fs::OpenFlags::F_Text);
    clearFile->close();
    raw_fd_ostream *clearFile2 = new raw_fd_ostream(path2.c_str(), ec, sys::fs::OpenFlags::F_Text);
    clearFile2->close();
  ...
  for (unsigned i = 0, e = MRI->getNumVirtRegs(); i != e; ++i) {
    ...
    if (Virt2PhysMap[Reg] != (unsigned)VirtRegMap::NO_PHYS_REG) {
      ...
      //bobo [factArray] version
        (*outFile) << '[' << PrintReg(Reg, TRI) << " -> "
        << PrintReg(Virt2PhysMap[Reg], TRI) << "] "
        << TRI->getRegClassName(MRI->getRegClass(Reg)) << "\n";
      // [factInt] or bool version
        (*outFile2) << '[' << PrintReg(Reg, TRI) << " -> "
        << PrintReg(Virt2PhysMap[Reg], TRI) << "] "
        << TRI->getRegClassName(MRI->getRegClass(Reg)) << "\n";
    }
  }
...
bool VirtRegRewriter::runOnMachineFunction(MachineFunction &fn) {
  ...
  //bobo
    ...
    (*outFile) << "********** REWRITE VIRTUAL REGISTERS **********\n"
    << "********** Function: " << MF->getName() << '\n';
    outFile->close();
    
    (*outFile2) << "********** REWRITE VIRTUAL REGISTERS **********\n"
    << "********** Function: " << MF->getName() << '\n';
    outFile2->close();
      ...
      if (MI->isIdentityCopy()) {
        ...
        if (MI->getNumOperands() == 2) {
          DEBUG(dbgs() << "Deleting identity copy.\n");
            ...
            raw_fd_ostream *outFile = new raw_fd_ostream(path.c_str(), ec, sys::fs::OpenFlags::F_Append);
            raw_fd_ostream *outFile2 = new raw_fd_ostream(path2.c_str(), ec, sys::fs::OpenFlags::F_Append);
            (*outFile) << "Deleting identity copy.\n";
            (*outFile2) << "Deleting identity copy.\n";
