...
void RegisterCoalescer::updateRegDefsUses(unsigned SrcReg,
  ...
  for (MachineRegisterInfo::reg_instr_iterator
    ...
    // bobo: read in the sub-register won't be regarded as a read instr
...
RegisterCoalescer::copyCoalesceInMBB(MachineBasicBlock *MBB) {
  ...
  if (JoinGlobalCopies) {
    ...
    for (MachineBasicBlock::iterator MII = MBB->begin(), E = MBB->end();
         MII != E; ++MII) {
      //bobo eliminate the default interval coalescing involved with the sensitive virtual virables
        MachineInstr *MI_test = &(*MII);
        bool delete_flag = false;
        
        std::string s;
        
        for (unsigned j = 0; j < MI_test->getNumOperands(); ++j)
        {
            if(MI_test->getOperand(j).isReg())
            {
                unsigned regNo = MI_test->getOperand(j).getReg();
                if(TargetRegisterInfo::isVirtualRegister(regNo))
                {
                    unsigned reg_index = TargetRegisterInfo::virtReg2Index(regNo);
                    DEBUG(dbgs() << "LiveInterval Coalescing: Reg Number(for testing): " << PrintReg(regNo) << "(" << regNo << ")" << "\n");
                    DEBUG(dbgs() << "Register ID: " << reg_index << "\n");
                    // eliminate the coalescing with sensitive Vars
                    // In this sensitive case, if j=0, the register is copied into another new register which is not considered. Index 6 is what we choose.
                    //bobo newly added the "forBackend.txt", iterately find if there the reg_index is in the HD_SENSITIVE list
                    std::string toCheck = std::to_string(reg_index);
                    std::ifstream fin("forBackend.txt");
                    if(!(fin.peek() == std::ifstream::traits_type::eof())) // FOR BACKEND FILE IS NOT EMPTY
                    {
                        while(fin >> s)
                        {
                        //if(!j && (reg_index == 6))
                        // ewly changed: some vreg is not in the operand(0)
                        //if(!j && (!s.compare(toCheck)))
            ...
            // bobo TODO: eliminate the registers like %vreg5 %vreg8
            // Don't put these sensitive Vars into the LocalWorkList
        if(delete_flag)
          continue;
      if (!MII->isCopyLike())
        continue;
...
bool RegisterCoalescer::runOnMachineFunction(MachineFunction &fn) {
  ...
  DEBUG(dbgs() << "********** SIMPLE REGISTER COALESCING **********\n"
               << "********** Function: " << MF->getName() << '\n');
  //bobo
    std::string path = "memOperand.log";
    std::string path2 = "memOriginal.log";
    std::string path3 = "memBool.log";
    std::error_code ec;
    raw_fd_ostream *outFile = new raw_fd_ostream(path.c_str(), ec, sys::fs::OpenFlags::F_Append);
    raw_fd_ostream *outFile2 = new raw_fd_ostream(path2.c_str(), ec, sys::fs::OpenFlags::F_Append);
    raw_fd_ostream *outFile3 = new raw_fd_ostream(path3.c_str(), ec, sys::fs::OpenFlags::F_Append);
    (*outFile) << "********** SIMPLE REGISTER COALESCING **********\n"
    << "********** Function: " << MF->getName() << '\n';
    outFile->close();
    (*outFile2) << "********** SIMPLE REGISTER COALESCING **********\n"
    << "********** Function: " << MF->getName() << '\n';
    outFile2->close();
    (*outFile3) << "********** SIMPLE REGISTER COALESCING **********\n"
    << "********** Function: " << MF->getName() << '\n';
    outFile3->close();
