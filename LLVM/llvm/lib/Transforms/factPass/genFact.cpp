...
namespace assign{
    ...
    **/
    ...
    std::string genFact::binary_type(Value *to, Value *from1, Value *from2)
    {
        ...
        bool rand_1 = false;
        bool rand_2 = false;
        bool key_sen_1 = false;
        bool key_sen_2 = false;
        bool key_ind_1 = false;
        bool key_ind_2 = false;


        if(RAND_var.find(from1_num) != RAND_var.end())
        {
            rand_1 = true;
            RAND_var.erase(from1_num);
        }else if(KEY_SENSITIVE_var.find(from1_num) != KEY_SENSITIVE_var.end())
        {
            key_sen_1 = true;
            KEY_SENSITIVE_var.erase(from1_num);
        }else if(KEY_IND_var.find(from1_num) != KEY_IND_var.end())
        {
            key_ind_1 = true;
            KEY_IND_var.erase(from1_num);
        }
        
        if(RAND_var.find(from2_num) != RAND_var.end())
        {
            rand_2 = true;
            RAND_var.erase(from2_num);
        } else if(KEY_SENSITIVE_var.find(from2_num) != KEY_SENSITIVE_var.end())
        {
            key_sen_2 = true;
            KEY_SENSITIVE_var.erase(from2_num);
        }else if(KEY_IND_var.find(from2_num) != KEY_IND_var.end())
        {
            key_ind_2 = true;
            KEY_IND_var.erase(from2_num);
        }
        std::string ret;
        ...
        if(!opName.compare("xor"))
        {
            if(rand_1 || rand_2)
            {
                if((rand_1 && (!BV_DIFF_REC(from1_inst, from2_inst))) || (rand_2 && (!BV_DIFF_REC(from2_inst, from1_inst))))
                {
                    RAND_var.insert(std::pair<unsigned, bool>(to_num, true));
                    ret = "RAND #x" + ToBitVecForm(to_num, input_s) + "\n";
                    // as for the array, I have another thread to process
                }else if(BV_SAME_REC(from1_inst, from2_inst))
                {
                    KEY_IND_var.insert(std::pair<unsigned, bool>(to_num, true));
                    ret = "KEY_IND #x" + ToBitVecForm(to_num, input_s) + "\n";
                }else
            ...
            }else if(key_ind_1 && key_ind_2)
            {
                ...
                return ret;
                /** [KEY_IND] + [KEY_SENSITIVE] **/
            }else if((key_ind_1 && key_sen_2) || (key_ind_2 && key_sen_1)){
                KEY_SENSITIVE_var.insert(std::pair<unsigned, bool>(to_num, true));
                ret = "KEY_SENSITIVE #x" + ToBitVecForm(to_num, input_s) + "\n";
                return ret;
            }
            ...
            /** [RAND] + [SID] **/
            if((key_ind_1 && rand_2) || (key_ind_2 && rand_1))
            {
                if(BV_ALL_INTERSECT(from1_inst, from2_inst))
                {
                    RAND_var.insert(std::pair<unsigned, bool>(to_num, true));
                ...
                }else if(BV_SAME_REC(from1_inst, from2_inst))
                {
                    KEY_IND_var.insert(std::pair<unsigned, bool>(to_num, true));
                    ret = "KEY_IND #x" + ToBitVecForm(to_num, input_s) + "\n";
                }else if((rand_1 && (!BV_DIFF_REC(from1_inst, from2_inst))) || (rand_2 && (!BV_DIFF_REC(from2_inst, from1_inst))))
                {
                    KEY_IND_var.insert(std::pair<unsigned, bool>(to_num, true));
                    ret = "KEY_IND #x" + ToBitVecForm(to_num, input_s) + "\n";
                }
            ...
            {
                if((!BV_DIFF_REC(from1_inst, from2_inst)) || (!BV_DIFF_REC(from2_inst, from1_inst)))
                {
                    KEY_IND_var.insert(std::pair<unsigned, bool>(to_num, true));
                    ret = "KEY_IND #x" + ToBitVecForm(to_num, input_s) + "\n";
                }else if(BV_SAME_REC(from1_inst, from2_inst))
