...
namespace assign{
    genFact::genFact(raw_fd_ostream *os, std::map<Instruction*, int> IDMap, std::vector<std::string> RAND_VAR, std::vector<std::string> CONSTANT_VAR, std::vector<std::string> KEY_VARi, unsigned input_s, unsigned input_bitset)
    {
    ...
    std::string genFact::createStore(Value *from1, Value *from2)
    {
        ...
        if(type_v->hasName())
        {
            ...
            if(std::find(RAND_VAR.begin(), RAND_VAR.end(), var_name) != RAND_VAR.end())
            {
                ...
                unsigned rand_var_id = std::find(RAND_VAR.begin(), RAND_VAR.end(), var_name) - RAND_VAR.begin();
                rand_var_id = 1 << (rand_var_id);
                std::string rand_var = "#b" + ToBitVecForm(rand_var_id, input_bitset);
                fact = fact + "(rule (RAND_VAR" + ' ' + fromString + ' ' + rand_var + "))" + "\n";
                fact = fact + "(rule (ALL_INCLUDE" + ' ' + fromString + ' ' + rand_var + "))";
                return fact;
            }else if(std::find(CONSTANT_VAR.begin(), CONSTANT_VAR.end(), var_name) != CONSTANT_VAR.end())
            {
                fact = ';' + var_name + "==>" + ' ' + "type" + "\n";
                fact = fact + "(rule (CONSTANT" + ' ' + fromString + "))" + "\n";
                unsigned cons_var_id = std::find(CONSTANT_VAR.begin(), CONSTANT_VAR.end(), var_name) - CONSTANT_VAR.begin();
                cons_var_id = 1 << (cons_var_id + r_s + k_s);
                std::string cons_var = "#b" + ToBitVecForm(cons_var_id, input_bitset);
                std::string zero_var = "#b" + ToBitVecForm(0, input_bitset);
                fact = fact + "(rule (RAND_VAR" + ' ' + fromString + ' ' + zero_var + "))" + "\n";
                fact = fact + "(rule (ALL_INCLUDE" + ' ' + fromString + ' ' + cons_var + "))";
            }else{ 
                /** belongs to the key set **/
                fact = ';' + var_name + "==>" + ' ' + "type" + "\n";
                fact = fact + "(rule (KEY_SENSITIVE" + ' ' + fromString + "))" + "\n";
                unsigned key_var_id = std::find(KEY_VAR.begin(), KEY_VAR.end(), var_name) - KEY_VAR.begin();
                key_var_id = 1 << (key_var_id + r_s);
                /** intial convert version **/
                //std::string key_var = "#b" + std::bitset<input_s>(key_var_id).to_string();
                std::string key_var = "#b" + ToBitVecForm(key_var_id, input_bitset);
                std::string zero_var = "#b" + ToBitVecForm(0, input_bitset);
                fact = fact + "(rule (RAND_VAR" + ' ' + fromString + ' ' + zero_var +  "))" + "\n";
                fact = fact + "(rule (ALL_INCLUDE" + ' ' + fromString + ' ' + key_var + "))";
            }
            return fact;
        }
    } 
